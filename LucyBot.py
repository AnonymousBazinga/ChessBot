...
def setboard():
    ...
    numbero = {'1': "the ", '2': "the second "}
    engche = {i: str(numbero[i[2]]) + str(colours[i[0]]) + str(notation[i[1]][0]) for i in vardict.values()}
    while variant_ref<16:
        win=tk.Tk()
        v28 = tkFont.Font(family='Times New Roman', size=28)
        win.title("LucyBot Set Chessboard")
        ...
        for i in range(1,9):
            for j in range(1,9):
                ...
                if (i+j)%2==0:
                    ...
                    btns[btn]=[j,i]
        txtlab=tk.Label(win,text="Place {}".format(engche[vardict[variant_ref]]),font=v28)
...
def update(i,j):
    ...
    global win
    win.destroy()
    if variant_ref%2==1 and [i,j] not in white.values() and [i,j] not in black.values() and j<3:
        white[vardict[variant_ref]]=[i,j]
        variant_ref+=1
        #print("hereeee1")
        #print(white)
    elif variant_ref%2==0 and [i,j] not in white.values() and [i,j] not in black.values() and j>6:
...
def anotherGoodMove(piece_notation, finalPos):
        ...
        if movecounter < 5:
            if movecounter==0:
                if (piece_notation, finalPos) == ('WP4',[4,4]):
                    netGain += 0.2
            elif movecounter==1:
                if (piece_notation,finalPos) == ('WN2',[6,3]):
                    netGain += 0.2
            elif (piece_notation, finalPos) == ('WP5',[5,3]) or (piece_notation, finalPos) == ('WP3',[3,4]):
                netGain += 0.1
        ...
        if piece_notation[1] == "B" or piece_notation[1] == "N":
            if white[piece_notation] == w123[piece_notation]:
                netGain += 0.09
            if movecounter < 20:
                if finalPos[0] == 8 or finalPos[1]==1 or finalPos[0] == 1:
                    netGain-= 0.08
...
def leads():
    ...
    main=tk.Tk()
    v11 = tkFont.Font(family='Calibri', size=11)
    main.title("LucyBot Scoreboard")
...
def takeusername():
    ...
    txtin=tk.Text(usrpop,height=2,width=20)
...
def helpfun():
    ...
    window.title("Help")
    v15 = tkFont.Font(family='Calibri', size=15)
...
def menu(): #main
    ...
    global usrn
    root=tk.Tk()
    v28 = tkFont.Font(family='Courier New', size=28)
    v15 = tkFont.Font(family='Calibri', size=15)
    v152 = tkFont.Font(family='Times New Roman', size=15)
    root.title("LucyBot Menu")
    root.geometry("568x592")

    text_wlcm=tk.Label(root,text="LucyBot!\n",font=("Times New Roman",25))
...
def UserMove(piece_notation,finalPos):
    ...
    global n
    global flagv
    initialPos1 = black[piece_notation]
    ...
    elif piece_notation[1]=="K" and (finalPos == [2, 8] or finalPos == [6, 8]) and initialPos1 == [4, 8]:
...
def HinCheck():
    if len(WHITESquareControl(black['BK1'])) != 0:
        return True
...
def covercode():
    ...
    while True:
        ...
        if Megalist != []:
            ...
            if temp1[2]>10000 or temp1[2]<-900:
                if HinCheck():
                    closing=tk.Tk()
                    closing.geometry("400x70")
                    closing.title("Result")
                    v28 = tkFont.Font(family='Times New Roman', size=20)
                    t1=tk.Label(text="Checkmate! You Lost",font=v28)
                    t1.pack()
                    playerwin=False
                    updatecsv()
                    ...
                    win=tk.Tk()
                    v28 = tkFont.Font(family='Times New Roman', size=28)
                    win.title("LucyBot Chessboard")
                    win.geometry("240x256")
                    buttonframe=tk.Frame(win)
                    createboard()
